...
class ArtifactDeliveryRuntime:
    def __init__(
        ...
        pdf_artifact_renderer: PdfArtifactRenderer,
        workspace_root: str,
    ) -> None:
        self._database = database
        self._gateway = model_gateway
        self._browser = browser_verifier
        self._pdf = pdf_artifact_renderer
    ...
    async def build_html(self, run_id: str) -> TaskWorkspaceRead:
        async with self._database.session() as session:
            ...
            markdown = self._markdown(task.goal, claims, citations, pages)
            ...
            html_ids = self._artifact_ids(run_id, "index.html", html_content)
            markdown_ids = self._artifact_ids(run_id, "report.md", markdown_content)
            artifact_items: list[tuple[str, str, bytes, dict[str, str]]] = [
                ("index.html", "text/html", html_content, html_ids)
            ]
            render_evidence: dict[str, PdfRenderVerificationRead] = {}
            if ".md" in workspace_policy.allowed_extensions:
                artifact_items.append(
                    ("report.md", "text/markdown", markdown_content, markdown_ids)
                )
            html_blob_path = self._write_blob(
                html_ids["workspace_id"],
                html_ids["digest"],
                html_content,
                suffix=".html",
            )
            if ".pdf" in workspace_policy.allowed_extensions:
                try:
                    rendered_pdf = await self._pdf.render(html_blob_path)
                except PdfArtifactRendererError as error:
                    raise ArtifactDeliveryVerificationError(
                        "PDF generation or real render verification failed"
                    ) from error
                pdf_ids = self._artifact_ids(run_id, "report.pdf", rendered_pdf.content)
                if rendered_pdf.verification.source_digest != pdf_ids["digest"]:
                    raise ArtifactDeliveryProofRejectedError(
                        "PDF render evidence is not bound to generated bytes"
                    )
                artifact_items.append(
                    ("report.pdf", "application/pdf", rendered_pdf.content, pdf_ids)
                )
                render_evidence["report.pdf"] = rendered_pdf.verification
            artifacts = tuple(artifact_items)
            if workspace_policy.max_files < len(artifacts):
                raise ArtifactDeliveryProofRejectedError("Workspace file quota is too small")
            required_extensions = {Path(path).suffix for path, _, _, _ in artifacts}
            if not required_extensions.issubset(set(workspace_policy.allowed_extensions)):
                raise ArtifactDeliveryProofRejectedError(
                    "Workspace extensions reject Builder output"
                )
            total_bytes = sum(len(content) for _, _, content, _ in artifacts)
            if total_bytes > workspace_policy.max_total_bytes:
                raise ArtifactDeliveryProofRejectedError("Artifacts exceed Workspace quota")
            blob_paths: dict[str, Path] = {}
            for relative_path, _, content, identifiers in artifacts:
                suffix = cast(Literal[".html", ".md", ".pdf"], Path(relative_path).suffix)
                ...
                if self._file_digest(blob_path) != identifiers["digest"]:
                    raise ArtifactDeliveryProofRejectedError("Workspace blob digest mismatch")
                blob_paths[relative_path] = blob_path
        ...
        async with self._database.session() as session, session.begin():
            ...
            workspace = TaskArtifactWorkspaceRecord(
                workspace_id=html_ids["workspace_id"],
            ...
            await session.flush()
            for relative_path, media_type, content, identifiers in artifacts:
                ...
                session.add(
                    ArtifactRevisionRecord(
                        ...
                        byte_count=len(content),
                        blob_name=blob_paths[relative_path].name,
                        patch_receipt_id=receipt.patch_receipt_id,
                        render_evidence=(
                            render_evidence[relative_path].model_dump(mode="json")
                            if relative_path in render_evidence
                            else None
                        ),
                        render_evidence_digest=(
                            render_evidence[relative_path].evidence_digest
                            if relative_path in render_evidence
                            else None
                        ),
                        created_at=now,
    ...
    def _html(
        ...
        return (
            ...
            "print-color-adjust:exact}main{max-width:none;padding:0}article{break-inside:avoid}"
            "a::after{content:' (' attr(href) ')';font-size:.78em;overflow-wrap:anywhere}}"
    ...
    def _markdown(
        ...
        sources: list[PageSnapshot] = []
        claim_blocks: list[str] = []
        for index, claim in enumerate(claims, 1):
            links: list[str] = []
            for citation in citation_by_claim[claim.claim_id]:
                page = pages[citation.page_snapshot_id]
                if page.page_snapshot_id not in source_numbers:
                    source_numbers[page.page_snapshot_id] = len(sources) + 1
                    sources.append(page)
                source_no = source_numbers[page.page_snapshot_id]
                links.append(
                    f"[来源 {source_no}]"
                    f"({ArtifactDeliveryRuntime._markdown_url(page.final_url)})"
                )
            claim_blocks.append(
                f"## 结论 {index}\n\n"
                f"{ArtifactDeliveryRuntime._markdown_text(claim.statement)}\n\n"
                f"{' · '.join(links)}"
            )
        source_items = [
            f"{index}. [{ArtifactDeliveryRuntime._markdown_text(page.title or page.final_url)}]"
            f"({ArtifactDeliveryRuntime._markdown_url(page.final_url)})"
            for index, page in enumerate(sources, 1)
        ]
        rendered_claims = "\n\n".join(claim_blocks)
        rendered_sources = "\n".join(source_items)
        return (
            f"# {ArtifactDeliveryRuntime._markdown_text(goal)}\n\n"
            "仅收录通过独立核验的 Claim 与其 Citation。\n\n"
            f"{rendered_claims}\n\n"
            "## 来源\n\n"
            f"{rendered_sources}\n"
        )
    ...
    @staticmethod
    def _artifact_ids(run_id: str, relative_path: str, content: bytes) -> dict[str, str]:
        workspace_id = f"wsp_{sha256_digest({'run_id': run_id})}"
        artifact_id = (
            f"art_{sha256_digest({'workspace_id': workspace_id, 'path': relative_path})}"
        )
        digest = ArtifactDeliveryRuntime._bytes_digest(content)
        revision_id = (
    ...
    def _write_blob(
        ...
        *,
        ...
        path = resolved_directory / f"{digest}{suffix}"
        try:
    ...
    async def _workspace_read(
        ...
        for artifact in artifacts:
            ...
            pdf_render_verification = None
            if revision.media_type == "application/pdf":
                if (
                    revision.render_evidence is None
                    or revision.render_evidence_digest is None
                ):
                    raise ArtifactDeliveryProofRejectedError("PDF render evidence drifted")
                pdf_render_verification = PdfRenderVerificationRead.model_validate(
                    revision.render_evidence
                )
                if (
                    pdf_render_verification.evidence_digest
                    != revision.render_evidence_digest
                    or pdf_render_verification.source_digest != revision.content_digest
                ):
                    raise ArtifactDeliveryProofRejectedError(
                        "PDF render evidence source drifted"
                    )
            elif (
                revision.render_evidence is not None
                or revision.render_evidence_digest is not None
            ):
                raise ArtifactDeliveryProofRejectedError(
                    "Non-PDF Artifact cannot carry PDF render evidence"
                )
            ...
            items.append(
                ArtifactRead(
                    ...
                    active_revision=ArtifactRevisionRead(
                        ...
                        media_type=cast(
                            Literal[
                                "application/pdf", "text/html", "text/css", "text/markdown"
                            ],
                            revision.media_type,
                        ),
                        ...
                        pdf_render_verification=pdf_render_verification,
