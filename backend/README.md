...
- `domain/provider_probe_authorizations.py` 与 `application/provider_probe_authorization.py` 冻结三家探针预算、零重试、24 小时 operator binding 和不解析凭据的离线 readiness report；CLI 只有 `manifest/preflight`，不提供 live run。
