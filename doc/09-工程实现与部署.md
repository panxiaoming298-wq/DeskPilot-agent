...
多 Agent Runtime 的 RuntimeWorkItem、Scheduler、Worker role、设备亲和性和 Embedded/Desktop/PostgreSQL 部署候选方案见[《多 Agent Scheduler 与部署拓扑技术设计》](多Agent-Scheduler与部署拓扑技术设计.md)。本文保留项目整体工程布局与早期部署原则。
...
`domain` 不依赖 FastAPI、Vue Flow、ELK.js、SQLAlchemy 或 OpenAI SDK。基础设施通过 application 定义的 port 接口注入；图布局与 UI 不进入 Runtime 真值。
...
| 编排 | 自研领域 Runtime | 复用已验证的 PostgreSQL claim/lease/fence、Effect Ledger、receipt/reconciliation 与 Verification | LangGraph 不进入核心，见 ADR-014 |
...
| 图可视化 | Vue Flow、ELK.js（`elkjs`）；Mermaid 导出 | 交互渲染、分层自动布局与静态分享职责分离 | 只读取服务端 GraphViewProjection，不执行工作流 |
...
关键决定使用 ADR 记录；早期列表仍有候选项，已接受项必须链接到实际文件：

- [ADR-014：图可视化与 LangGraph 采用边界](ADR-014-图可视化与LangGraph采用边界.md)：核心 Runtime 不采用 LangGraph，Execution Graph 使用只读投影 + Vue Flow + ELK.js。
