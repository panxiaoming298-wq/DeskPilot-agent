...
这些护栏现已由 [Runner 离线实现检查点](115B-Provider探针Runner离线实现.md) 落成独立 library：新增最长 15 分钟的一次性执行许可、持久 claim、逐请求保守预算预留和脱敏 receipt/report，并以 MockTransport 验证三家各 4 次的串行执行。[Live CLI 接线检查点](115B-Provider探针LiveCLI接线.md) 又增加默认拒绝、CI 禁令、环境开关、exact run-id 和不可覆盖报告门；接线没有被运行。readiness report 仍固定 `network_access=false`、`credentials_resolved=false`、`real_model_capture=false`、`production_admission=false`、`cloud_activation=false`。
...
4. live runner 与 CLI 已完成离线接线，但实际运行仍必须按 Provider 再次单独授权。实现不等于运行。
