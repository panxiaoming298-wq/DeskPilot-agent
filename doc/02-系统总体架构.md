# 02. 系统总体架构

> 方向更新：本文保留基础容器和分层设计；阶段 69 之后的权威通用任务架构见[《通用对话、联网研究与 Artifact 工作区总体架构》](通用对话联网研究与Artifact工作区总体架构.md)，产品边界见 [ADR-015](ADR-015-通用任务Agent产品边界与首个纵向切片.md)。当前代码尚未实现 Web Research/Artifact/Browser 纵向闭环。

## 1. 架构原则

1. **模型只提议，策略层授权，Runner 执行。**
2. **控制面与执行面分离。** API/编排进程不直接操作系统。
3. **状态显式。** 任务计划、步骤、审批、结果、错误都使用领域对象，不藏在聊天文本里。
4. **先确定性，后智能。** 路由、权限、参数校验、重试和验证尽量由代码负责。
5. **本地优先。** 文件原文与系统动作默认不出本机，云端只收到完成任务所需的最少上下文。
6. **端口可替换。** LLM、数据库、搜索、浏览器、OS 实现通过接口适配。

## 2. 系统上下文

```mermaid
flowchart TB
    USER["本机用户"]
    DP["DeskPilot"]
    LLM["云端 OpenAI-compatible API"]
    LOCAL["Ollama / 本地模型"]
    WEB["搜索引擎与网站"]
    WIN["Windows、文件、应用"]
    MCP["受信任 MCP Server"]

    USER <-->|"对话、审批、接管"| DP
    DP <-->|"最小必要上下文"| LLM
    DP <-->|"本机推理"| LOCAL
    DP <-->|"搜索、浏览"| WEB
    DP <-->|"受控工具调用"| WIN
    DP <-->|"发现并调用工具"| MCP
```

## 3. 容器级架构

```mermaid
flowchart TB
    subgraph Client["表现层"]
      FE["Vue 3 前端"]
      SHELL["Tauri 薄壳（后期）"]
    end

    subgraph Control["控制面：Python 主进程"]
      API["FastAPI REST/WebSocket"]
      CONV["Conversation / Task Contract"]
      APP["Application Services"]
      GRAPH["Task Graph / Supervisor"]
      ROUTER["Intent & Model Router"]
      POLICY["Policy / Approval Engine"]
      REG["Agent & Tool Registry"]
      EVENTS["Event Store / Outbox"]
    end

    subgraph Execution["执行面"]
      RUNNER["Tool Runner Process"]
      PAGE["Search / Page Reader"]
      BROWSER["Isolated Browser Verifier"]
      INDEXER["Document Indexer"]
      MCPH["MCP Host"]
    end

    subgraph Infra["基础设施"]
      DB["SQLite WAL"]
      VEC["Pluggable Vector Index"]
      CREDS["Windows Credential Manager"]
      MODELS["Model Providers"]
      ART["Task Artifact Store"]
    end

    FE <--> API
    SHELL --- FE
    API --> CONV --> APP --> GRAPH
    GRAPH --> ROUTER
    GRAPH --> POLICY
    GRAPH --> REG
    GRAPH --> EVENTS
    POLICY --> RUNNER
    POLICY --> PAGE
    GRAPH --> ART
    ART --> BROWSER
    RUNNER --> INDEXER
    REG --> MCPH
    ROUTER --> MODELS
    EVENTS --> DB
    INDEXER --> DB
    INDEXER --> VEC
    APP --> CREDS
```

## 4. 组件职责

| 组件 | 职责 | 不负责 |
| --- | --- | --- |
| FastAPI | 鉴权、资源 API、事件连接、输入校验 | 不直接调用 OS |
| Application Services | 用例编排、事务边界、DTO 转换 | 不包含模型提示词 |
| Supervisor/Task Graph | 任务状态、Agent 调度、步骤依赖、恢复 | 不决定最终权限 |
| Intent Router | 快速路径/复杂路径分类、所需能力识别 | 不执行动作 |
| Model Gateway | Provider 适配、能力协商、限流、重试、用量 | 不做业务授权 |
| Agent Registry | Agent Profile 发现、版本和工具白名单 | 不持有运行状态 |
| Tool Registry | 工具 schema、风险、实现、版本、健康状态 | 不信任工具自报风险 |
| Policy Engine | 决策 allow/deny/ask、审批绑定、数据边界 | 不调用模型决定权限 |
| Tool Runner | 在受限环境执行、超时/取消、采集结果 | 不接受未签名调用 |
| Verifier | 验证产物、前后状态和完成条件 | 不凭聊天文本判断成功 |
| Event Store | 追加任务事实、恢复、回放、审计 | 不保存明文密钥/思维链 |
| Indexer | 文档提取、切块、索引、增量更新 | 不默认全盘扫描 |
| Search/Page Reader | 受控搜索、SSRF/出境防护、页面快照 | 不执行网页指令或使用用户登录态 |
| Task Artifact Store | 单任务工作区、不可变 revision、patch/export receipt | 不允许 Agent 直接写用户项目 |
| Browser Verifier | 默认断网渲染、DOM/error/network/screenshot 证据 | 不修改 Artifact、不复用用户 Cookie |

## 5. 分层设计

采用整洁架构/六边形思想，依赖方向从外向内：

```text
interfaces (FastAPI/WebSocket/CLI)
        ↓
application (use cases, commands, queries)
        ↓
domain (Task, Step, ToolCall, Approval, PolicyDecision)
        ↑
infrastructure (SQLite, OpenAI, Ollama, Playwright, Win32, MCP)
```

领域层只依赖 Python 标准库和少量类型协议。核心状态图由项目自身的 application/domain Runtime 执行，Task/Plan generation/Node/Invocation/Event/Effect/Verification 保持领域身份和唯一真值。面向用户的图由只读 `GraphViewProjection` 生成，前端布局不进入领域层；LangGraph 不作为核心 Runtime，边界见 [ADR-014](ADR-014-图可视化与LangGraph采用边界.md)。

## 6. 进程模型

### 6.1 MVP

| 进程 | 数量 | 说明 |
| --- | --- | --- |
| API + Orchestrator | 1 | 本机 `127.0.0.1` 监听，管理对话与任务 |
| Tool Runner | 1 | 通过本地 IPC 接受已授权调用 |
| Browser Worker | 0/1 | 首次浏览器任务时启动 |
| Index Worker | 0/1 | 后台增量索引，低优先级 |
| Vue Dev Server | 1 | 仅开发期；发布时为静态资源/桌面壳 |

Runner 崩溃不应带走 API；API 重启后从数据库恢复等待中或可重试步骤。MVP 不引入 Redis/Celery，避免个人项目运维过重。

### 6.2 扩展阶段

需要远程执行、多设备或高并发后，再将事件总线换成 Redis/NATS、数据库换 PostgreSQL、Runner 注册为远程 Worker。领域协议保持不变。

## 7. 核心数据流

### 7.1 只读快速任务

```mermaid
sequenceDiagram
    participant U as 用户
    participant F as 前端
    participant O as Orchestrator
    participant P as Policy
    participant R as Runner
    U->>F: “我的 C 盘还剩多少空间？”
    F->>O: 创建任务
    O->>O: 路由为只读快速路径
    O->>P: 请求 computer.disk_usage
    P-->>O: allow(R0)
    O->>R: 已签名工具调用
    R-->>O: 结构化磁盘数据
    O-->>F: 结果 + 数据来源 + 完成事件
```

简单任务可以不调用规划模型；必要时仅用便宜模型把结构化结果解释成人话。

### 7.2 复杂且有副作用的任务

```mermaid
sequenceDiagram
    participant U as 用户
    participant O as Supervisor
    participant M as Model Gateway
    participant A as Specialist Agents
    participant P as Policy/Approval
    participant R as Runner
    participant V as Verifier
    U->>O: 提交目标
    O->>M: 请求结构化计划
    M-->>O: Task DAG
    O->>A: 分派就绪步骤
    A->>P: 提议工具调用
    P-->>U: 精确风险预览
    U-->>P: 同意一次
    P->>R: 签发参数绑定授权
    R-->>A: 执行结果 + 证据
    A->>V: 请求验证
    V-->>O: 完成/修复/失败
    O-->>U: 汇总结果与未完成项
```

## 8. 同步与异步边界

- REST 创建任务后立即返回 `task_id`，不等待模型或工具完成。
- WebSocket 订阅任务事件；断线后用 `after_seq` 补拉事件。
- 数据库写入任务事件和 outbox 在同一事务中，事件推送失败可重发。
- 模型、浏览器、索引和工具执行都有独立 timeout/cancellation token。
- 大文件解析返回 artifact 引用，避免把全部内容通过 WebSocket 推送。

## 9. 部署与网络边界

- API 默认只绑定 `127.0.0.1`，使用每次启动生成的本地会话令牌防止其他网页调用。
- 前端若由桌面壳加载，使用受限 IPC 或带 Origin 校验的本地 HTTP。
- 所有外连经 Egress Policy：模型域名、搜索域名、MCP server 分别配置。
- 密钥不写入普通配置文件；发布形态使用 Windows Credential Manager。
- 日志、数据库和索引目录可在设置页查看与清理。

## 10. 为什么该架构可行

- Python 生态已有 FastAPI、Pydantic、Playwright、文档解析和 Windows 自动化库，不需要从零实现底层能力。
- [LangGraph 官方概览](https://docs.langchain.com/oss/python/langgraph/overview)说明其长时有状态执行、持久化、流式和 human-in-the-loop 能力；项目已完成对比，但现有领域 Runtime 的 lease/fence、Effect Ledger、unknown/reconciliation 和 Verification 不应为可视化迁移成第二套 checkpoint 真值。
- [Vue Flow](https://vueflow.dev/)与 [ELK Layered](https://eclipse.dev/elk/reference/algorithms/org-eclipse-elk-layered.html)分别承担交互图和显示布局，并通过服务端只读投影与 Runtime 解耦。
- [OpenAI Function calling 文档](https://developers.openai.com/api/docs/guides/function-calling)采用“模型产生结构化工具请求，应用执行并回传”的模式，与模型/执行隔离相符。
- [MCP 规范](https://modelcontextprotocol.io/specification/2025-06-18/server/index)为第三方 prompts/resources/tools 提供通用扩展面，但本项目在 MCP 外仍增加本地策略层。
- SQLite 和单机多进程足以支撑个人演示，避免过早引入分布式系统；接口已经为后期迁移预留。
